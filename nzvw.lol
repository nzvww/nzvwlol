...
do 
	...
	do 
		...
		do 
			...
			utility.drawings["Sub"] = utility:drawObject("Text", {Outline = true, Text = "nzvw", Color = Color3.new(1, 1, 1)})
			utility.drawings["Domain"] = utility:drawObject("Text", {Outline = true, Text = ".lol", Color = Color3.fromRGB(149, 62, 188)})
			--
		...
		do 
			...
			function HitReg:HitSound(Id, Volume, Pitch)
				...
				--[[writefile("song.mp3", game:HttpGet("https://nzvw.feet.media/osiris/cmwr7ecb.mp3"))
...
local Window = Library:Window({Size = UDim2.new(0,580,0,600)}) do 
	...
	local misc = Window:Page({Name = "Misc", Weapons = true}) do 
		...
		local chatSpam = Movement:Section({Name = "Chat Spam", Size = 330, Side = "Left"}) do
			...
			chatSpam:Dropdown({Name = "Type", Flag = "Chat Spam Type", Options = {"nzvw", "Scottish"}, Default = "nzvw"})
	...
	local Config = Window:Page({Name = "Settings"}) do
		...
		--
		local CurrentList = {};
		if not isfolder("nzvw") then 
			makefolder("nzvw")
		end 
		if not isfolder("nzvw/Configs") then 
			makefolder("nzvw/Configs")
		end 
		local function UpdateConfigList()
			local List = {};
			for idx, file in ipairs(listfiles("nzvw/Configs")) do
				local FileName = file:gsub("nzvw/Configs\\", ""):gsub(".cfg", "")
		...
		Cfgs:Button({Name = "Create", Callback = function()
			...
			if ConfigName == "" or isfile("nzvw/Configs/" .. ConfigName .. ".cfg") then
				return;
			end;
			writefile("nzvw/Configs/" .. ConfigName .. ".cfg", Library:GetConfig());
			UpdateConfigList();
		end})
		Cfgs:Button({Name = "Save", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				writefile("nzvw/Configs/" .. SelectedConfig .. ".cfg", Library:GetConfig())
			end;
		end})
		Cfgs:Button({Name = "Load", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				Library:LoadConfig(readfile("nzvw/Configs/" .. SelectedConfig .. ".cfg"))
			end
		end})
		Cfgs:Button({Name = "Delete", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				delfile("nzvw/Configs/" .. SelectedConfig .. ".cfg")
			end
			UpdateConfigList();
		end})
...
Library:Notification("Loaded nzvw.lol in " .. Floor((tick() - loadingTime) * 1000)  .."ms", 3, Library.Accent, flags["Notification Position"])
